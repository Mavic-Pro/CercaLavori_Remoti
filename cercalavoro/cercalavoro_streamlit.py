...
def main():
    ...
    if st.button("Search"):
        ...
        if not engines:
            ...
            # Get the content of the file as a byte stream
            output_content = engine.get_output_content(output_format)

            # Save the content to a local file
            local_file_path = f"./{output_filename}.{output_format}"
            with open(local_file_path, "wb") as local_file:
                local_file.write(output_content)
            ...
            if download_button:
                st.success(f"Search complete. Results saved locally to {local_file_path}")
